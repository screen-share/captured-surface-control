...
With most browsers' interpretation of [Permissions Policy](https://www.w3.org/TR/permissions-policy-1/#permissionspolicy), the first time an origin invokes either `captureWheel()` or `setZoomLevel()`, the browser shows a [permission prompt](https://developer.mozilla.org/en-US/docs/Web/API/Permissions). How long this permission may be persisted is up to the browser, with typical durations being "forever" or "for the current browsing session".
